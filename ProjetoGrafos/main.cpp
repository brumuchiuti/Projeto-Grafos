...
/*
  ...
  - Vértices: id (0..n-1) e "apelido" (nome)
  ...
  e) Remover vértice
  f) Remover aresta
  g) Mostrar conteúdo do arquivo (grafo.txt)
  h) Mostrar grafo (lista/matriz de adjacência)
  i) Conexidade (C3/C2/C1/C0) + grafo reduzido (SCC)
  j) Encerrar
*/
...
struct Graph {
    ...
    void removeVertex(int x) {
        ...
        // 1) remove edges que chegam em x
        for(auto &vec: adj){
            vec.erase(remove_if(vec.begin(), vec.end(),
                                [&](const Edge& e){ return e.to==x; }),
                      vec.end());
        }
        // 2) apaga o x-ésimo vértice e sua lista
        adj.erase(adj.begin()+x);
        label.erase(label.begin()+x);
        // 3) reindexa destinos > x (dest--), pois ids mudaram
        for(auto &vec: adj){
            for(auto &e: vec){
                if(e.to > x) e.to--;
            }
    ...
    bool loadFromFile(const string& path) {
        ...
        for(int i=0;i<nverts;i++){
            ...
            // formato esperado: id "Nome com espacos"
            // vamos parsear: primeiro int, depois aspas
            // Ex.: 0 "Hospital X"
            stringstream ss(line);
            int id;
            ss >> id;
            ...
            if(firstQuote != string::npos && lastQuote != string::npos && lastQuote > firstQuote){
                ...
                // fallback: usa o resto do line apos o id
    ...
    static void showFilePretty(const string& path) {
        ifstream in(path);
        if(!in){ cout << "Nao foi possivel abrir: " << path << "\n"; return; }
        // só imprime com um layout mais amigável
        string line;
    ...
    // (opcional) matriz de adjacência impressa (peso ou INF)
    void showAdjMatrix() const {
        const double INF = 1e18;
        vector<vector<double>> M(n(), vector<double>(n(), INF));
        for(int i=0;i<n();i++) M[i][i]=0.0;
        for(int u=0;u<n();u++){
            for(const auto& e: adj[u]){
                M[u][e.to] = min(M[u][e.to], e.w);
            }
        }
        cout << "==== Matriz de adjacencia (INF = sem aresta) ====\n";
        cout << "     ";
        ...
        for(int i=0;i<n();i++){
            cout << setw(4) << i << " ";
            for(int j=0;j<n();j++){
                if(M[i][j] >= 1e18/2) cout << setw(7) << "INF";
                else cout << setw(7) << fixed << setprecision(1) << M[i][j];
            }
            cout << "   | " << label[i] << "\n";
        }
        cout << "================================================\n";
    ...
    // conectado fraco: considera não direcionado
    ...
    bool isUnilateral() const {
        int N=n();
        if(N<=1) return true;
        // Para economia, roda BFS/DFS de cada nó até cobrir pares;
        // aqui usamos BFS sobre digrafo (sem peso).
        for(int s=0; s<N; s++){
            vector<int> vis(N,0);
            ...
            // marca quem ficou alcançável a partir de s
            // checa pares (s,x): se nem s->x nem x->s, falha
            for(int x=0;x<N;x++){
                if(x==s) continue;
                if(vis[x]) continue; // s->x ok
                // tenta x->s
                vector<int> vis2(N,0);
    ...
    void connectivityReportAndReduced() const {
        ...
        // C3: fortemente conexo (apenas 1 SCC)
        if(C==1){
            cout << "Conexidade (direcionado): C3 (fortemente conexo)\n";
        } else {
            // C2: unilateral (para todo par, existe caminho em pelo menos um sentido)
            if(isUnilateral()){
        ...
        // Construir grafo reduzido (condensação)
...
void printMenu() {
    cout << "\nMENU\n"
         ...
         << "h) Mostrar grafo (lista e opcional matriz)\n"
...
int main() {
    ...
    while(true){
        ...
        if(op=='a' || op=='A'){
            ...
            Graph::showFilePretty(path);
        }
        else if(op=='h' || op=='H'){
            cout << "1) Lista de adjacencia  2) Matriz de adjacencia (opcional) -> ";
            int ch; cin >> ch;
            if(ch==2) G.showAdjMatrix();
            else G.showAdjList();
        }
        else if(op=='i' || op=='I'){
